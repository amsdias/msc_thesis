...
load ('saved_variables/vhclassify_c_1')
...
w_class = 0.1*randn(size(w1,2)+1,7);
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if epoch<6  % First update top-level weights holding other weights fixed. 
    ...
    VV = [w_class(:)']';
    Dim = [l4; l5];
    [X, fX] = minimize(VV,'CG_CLASSIFY_INIT_c_1',max_iter,Dim,w1probs,targets);
    w_class = reshape(X,l4+1,l5);

  else
    VV = [w1(:)' w_class(:)']';
    Dim = [l1; l4; l5];
    [X, fX] = minimize(VV,'CG_CLASSIFY_c_1',max_iter,Dim,data,targets);
...
%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 save(['saved_variables/classify_weights_c_1_' num2str(iter)], 'w1', 'w_class');
 save(['saved_variables/classify_error_c_1_' num2str(iter)], 'test_err', 'test_crerr', 'train_err', 'train_crerr');
