...
function [f, df] = CG_CLASSIFY_f_5(VVL, VVR,Dim,XX,target)
...
% Do decomversion.
 ...
 xxx = xxx+(l5+1)*l6;
 w_classL = reshape(VVL(xxx+1:xxx+(l6+1)*l7),l6+1,l7);
 w_classR = reshape(VVR(xxx+1:xxx+(l6+1)*l7),l6+1,l7);
...
Ix5L = (Ix_class*w_classL').*w5probs.*(1-w5probs);
Ix5R = (Ix_class*w_classR').*w5probs.*(1-w5probs);
Ix5 = (Ix5L + Ix5R)/2;
Ix5 = Ix5(:,1:end-1);
dw5 =  w4probs'*Ix5;
