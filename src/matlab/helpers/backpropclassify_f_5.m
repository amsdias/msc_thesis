...
load ('saved_variables/vhclassify_f_5')
load ('saved_variables/hpclassify_f_5')
load ('saved_variables/hp2classify_f_5')
load ('saved_variables/hp3classify_f_5')
load ('saved_variables/hp4classify_f_5')
...
w5L=[hidpen4L; penrecbiases4L];
w5R=[hidpen4R; penrecbiases4R];
w_classL = -0.01*rand(size(w5L,2)+1,7);
w_classR = 0.01*rand(size(w5R,2)+1,7);
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if epoch<6  % First update top-level weights holding other weights fixed. 
    ...
    VVR = [w_classR(:)']';
    Dim = [l6; l7];
    [XL, XR, fX] = minimizef(VVL, VVR, 'CG_CLASSIFY_INIT_f_5', max_iter, Dim, w5probs, targets);
    w_classL = reshape(XL,l6+1,l7);
    w_classR = reshape(XR,l6+1,l7);

  else
    VVL = [w1L(:)' w2L(:)' w3L(:)' w4L(:)' w5L(:)' w_classL(:)']';
    VVR = [w1R(:)' w2R(:)' w3R(:)' w4R(:)' w5R(:)' w_classR(:)']';
    Dim = [l1; l2; l3; l4; l5; l6; l7];
    [XL, XR, fX] = minimizef(VVL, VVR, 'CG_CLASSIFY_f_5', max_iter, Dim, data, targets);
    ...
    xxx = xxx+(l5+1)*l6;
    w_classL = reshape(XL(xxx+1:xxx+(l6+1)*l7),l6+1,l7);
    w_classR = reshape(XR(xxx+1:xxx+(l6+1)*l7),l6+1,l7);
...
%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 %save mnistclassify_weights w1L w1R w2L w2R w3L w3R w4L w4R w5L w5R w_classL w_classR
 %save mnistclassify_error test_err test_crerr train_err train_crerr;
 save(['saved_variables/classify_weights_f_5_' num2str(iter)], 'w1L', 'w1R', 'w2L', 'w2R', 'w3L', 'w3R', 'w4L', 'w4R', 'w5L', 'w5R', 'w_classL', 'w_classR');
 save(['saved_variables/classify_error_f_5_' num2str(iter)], 'test_err', 'test_crerr', 'train_err', 'train_crerr');
