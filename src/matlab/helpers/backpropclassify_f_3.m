...
load ('saved_variables/vhclassify_f_3')
load ('saved_variables/hpclassify_f_3')
load ('saved_variables/hp2classify_f_3')
...
w_classL = -0.01*rand(size(w3L,2)+1,7);
w_classR = 0.01*rand(size(w3R,2)+1,7);
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if epoch<6  % First update top-level weights holding other weights fixed. 
    ...
    VVR = [w_classR(:)']';
    Dim = [l4; l5];
    [XL, XR, fX] = minimizef(VVL, VVR, 'CG_CLASSIFY_INIT_f_3', max_iter, Dim, w3probs, targets);
    w_classL = reshape(XL,l4+1,l5);
    w_classR = reshape(XR,l4+1,l5);

  else
    VVL = [w1L(:)' w2L(:)' w3L(:)' w_classL(:)']';
    VVR = [w1R(:)' w2R(:)' w3R(:)' w_classR(:)']';
    Dim = [l1; l2; l3; l4; l5];
    [XL, XR, fX] = minimizef(VVL, VVR, 'CG_CLASSIFY_f_3', max_iter, Dim, data, targets);
...
%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 %save mnistclassify_weights w1L w1R w2L w2R w3L w3R w_classL w_classR
 %save mnistclassify_error test_err test_crerr train_err train_crerr;
 save(['saved_variables/classify_weights_f_3_' num2str(iter)], 'w1L', 'w1R', 'w2L', 'w2R', 'w3L', 'w3R', 'w_classL', 'w_classR');
 save(['saved_variables/classify_error_f_3_' num2str(iter)], 'test_err', 'test_crerr', 'train_err', 'train_crerr');
