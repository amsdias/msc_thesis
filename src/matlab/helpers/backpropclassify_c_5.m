...
load ('saved_variables/vhclassify_c_5')
load ('saved_variables/hpclassify_c_5')
load ('saved_variables/hp2classify_c_5')
load ('saved_variables/hp3classify_c_5')
load ('saved_variables/hp4classify_c_5')
...
w_class = 0.1*randn(size(w5,2)+1,7);
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if epoch<6  % First update top-level weights holding other weights fixed. 
    ...
    w4probs = 1./(1 + exp(-w3probs*w4));
    w4probs = [w4probs ones(N,1)];
    w5probs = 1./(1 + exp(-w4probs*w5)); %w3probs = [w3probs  ones(N,1)];

    VV = [w_class(:)']';
    Dim = [l6; l7];
    [X, fX] = minimize(VV,'CG_CLASSIFY_INIT_c_5',max_iter,Dim,w5probs,targets);
    w_class = reshape(X,l6+1,l7);

  else
      
    VV = [w1(:)' w2(:)' w3(:)' w4(:)' w5(:)' w_class(:)']';
    Dim = [l1; l2; l3; l4; l5; l6; l7];
    [X, fX] = minimize(VV,'CG_CLASSIFY_c_5',max_iter,Dim,data,targets);
    ...
    xxx = xxx+(l5+1)*l6;
    w_class = reshape(X(xxx+1:xxx+(l6+1)*l7),l6+1,l7);
...
%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 %save mnistclassify_weights w1 w2 w3 w4 w5 w_class
 %save mnistclassify_error test_err test_crerr train_err train_crerr;
 save(['saved_variables/classify_weights_c_5_' num2str(iter)], 'w1', 'w2', 'w3', 'w4', 'w5', 'w_class');
 save(['saved_variables/classify_error_c_5_' num2str(iter)], 'test_err', 'test_crerr', 'train_err', 'train_crerr');
