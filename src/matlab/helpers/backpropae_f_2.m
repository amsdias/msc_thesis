...
load ('saved_variables/vh_f_2l.mat')
load ('saved_variables/po_f_2l.mat')
...
w2L=[hidtopL; toprecbiasesL];
w2R=[hidtopR; toprecbiasesR];
w3L=[hidtopL'; topgenbiasesL]; 
w3R=[hidtopR'; topgenbiasesR];
w4L=[vishidL'; visbiasesL];
w4R=[vishidR'; visbiasesR];
...
for batch = 1:numbatches
  ...
  dataoutL = 1./(1 + exp(-w3probs*w4L));
  dataoutR = 1./(1 + exp(-w3probs*w4R));
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  max_iter=3;
  VVL = [w1L(:)' w2L(:)' w3L(:)' w4L(:)']';  %%do one or both sets at once?
  VVR = [w1R(:)' w2R(:)' w3R(:)' w4R(:)']';  %%do one or both sets at once?
  Dim = [l1; l2; l3; l4; l5];

  %[XL, fXL] = minimize(VVL,'CG_MNIST',max_iter,Dim,data);
  %[XR, fXR] = minimize(VVR,'CG_MNIST',max_iter,Dim,data);
  [XL, XR, fX] = minimizef(VVL, VVR, 'CG_AE_f_2', max_iter, Dim, data);
...
%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %save mnist_weights2f w1L w1R w2L w2R w3L w3R w4L w4R
  %save mnist_error2f test_err train_err;
  save(['saved_variables/weights_f_2l_' num2str(iter)], 'w1L', 'w1R', 'w2L', 'w2R', 'w3L', 'w3R', 'w4L', 'w4R');  
  save(['saved_variables/error_f_2l_' num2str(iter)], 'test_err', 'train_err');
