function [XL, XR, fX, i] = minimizef(XL, XR, f, length, varargin) %XL, XR - weigths, varargin - input
...
if max(size(length)) == 2, red=length(2); length=length(1); 
else
    red=1; 
end
...
[f0, df0] = feval(f, XL, XR, varargin{:});          % get function value and gradient
fX = f0;
...
while i < abs(length)                                      % while not finished
  ...
  while 1                             % keep extrapolating as long as necessary
    ...
    while ~success && M > 0
      try
        M = M - 1;
        i = i + (length<0);                         % count epochs?!
        [f3, df3] = feval(f, XL+x3*s, XR+x3*s, varargin{:});
    ...
    x3 = x1-d1*(x2-x1)^2/(B+sqrt(complex(B*B-A*d1*(x2-x1)))); % num. error possible, ok!   REMOVE THE COMPLEX() WHEN NOT USING GPU
  ...
  if abs(d3) < -SIG*d0 && f3 < f0+x3*RHO*d0          % if line search succeeded
    ...
    if d0 > 0                                      % new slope must be negative
      s = -df0;
      d0 = -s'*s;                  % otherwise use steepest direction
    end
