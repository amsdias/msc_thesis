...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if epoch<6  % First update top-level weights holding other weights fixed. 
    ...
    VV = [w_class(:)']';
    Dim = [l4; l5];
    [X, fX] = minimize(VV,'CG_CLASSIFY_INIT_c_0',max_iter,Dim,XX,targets);
    w_class = reshape(X,l4+1,l5);

  else
    VV = [w_class(:)']';
    Dim = [l4; l5];
    [X, fX] = minimize(VV,'CG_CLASSIFY_c_0',max_iter,Dim,data,targets);    
...
%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 %save mnistclassify_weights w_class
 %save mnistclassify_error test_err test_crerr train_err train_crerr;
 save(['saved_variables/classify_weights_c_0_' num2str(iter)], 'w_class');  
 save(['saved_variables/classify_error_c_0_' num2str(iter)], 'test_err', 'test_crerr', 'train_err', 'train_crerr');
