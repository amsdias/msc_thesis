...
load ('saved_variables/vh_f_4.mat')
load ('saved_variables/hp_f_4.mat')
load ('saved_variables/hp2_f_4.mat')
load ('saved_variables/po_f_4.mat')
...
w4L=[hidtopL; toprecbiasesL];
w4R=[hidtopR; toprecbiasesR];
w5L=[hidtopL'; topgenbiasesL]; 
w5R=[hidtopR'; topgenbiasesR];
w6L=[hidpen2L'; hidgenbiases2L];
w6R=[hidpen2R'; hidgenbiases2R]; 
w7L=[hidpenL'; hidgenbiasesL];
w7R=[hidpenR'; hidgenbiasesR];
w8L=[vishidL'; visbiasesL];
w8R=[vishidR'; visbiasesR];
...
for batch = 1:numbatches
  ...
  dataoutL = 1./(1 + exp(-w7probs*w8L));
  dataoutR = 1./(1 + exp(-w7probs*w8R));
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  max_iter=3;
  VVL = [w1L(:)' w2L(:)' w3L(:)' w4L(:)' w5L(:)' w6L(:)' w7L(:)' w8L(:)']';  %%do one or both sets at once?
  VVR = [w1R(:)' w2R(:)' w3R(:)' w4R(:)' w5R(:)' w6R(:)' w7R(:)' w8R(:)']';  %%do one or both sets at once?
  ...
  [XL, XR, fX] = minimizef(VVL, VVR, 'CG_AE_f_4', max_iter, Dim, data);
...
%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %save mnist_weights4f w1L w1R w2L w2R w3L w3R w4L w4R w5L w5R w6L w6R w7L w7R w8L w8R 
  %save mnist_error4f test_err train_err;
  save(['saved_variables/weights_f_4_' num2str(iter)], 'w1L', 'w1R', 'w2L', 'w2R', 'w3L', 'w3R', 'w4L', 'w4R', 'w5L', 'w5R', 'w6L', 'w6R', 'w7L', 'w7R', 'w8L', 'w8R');  
  save(['saved_variables/error_f_4_' num2str(iter)], 'test_err', 'train_err');
