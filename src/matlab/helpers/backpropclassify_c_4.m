...
load ('saved_variables/vhclassify_c_4')
load ('saved_variables/hpclassify_c_4')
load ('saved_variables/hp2classify_c_4')
load ('saved_variables/hp3classify_c_4')
...
w_class = 0.1*randn(size(w4,2)+1,7); 
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if epoch<6  % First update top-level weights holding other weights fixed. 
    ...
    VV = [w_class(:)']';    
    Dim = [l5; l6];
    [X, fX] = minimize(VV, 'CG_CLASSIFY_INIT_c_4', max_iter, Dim, w4probs, targets);
    w_class = reshape(X,l5+1,l6);    

  else
    VV = [w1(:)' w2(:)' w3(:)' w4(:)' w_class(:)']';    
    Dim = [l1; l2; l3; l4; l5; l6];
    [X, fX] = minimize(VV, 'CG_CLASSIFY_c_4', max_iter, Dim, data, targets);
...
%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 save(['saved_variables/classify_weights_c_4_' num2str(iter)], 'w1', 'w2', 'w3', 'w4','w_class');
 save(['saved_variables/classify_error_c_4_' num2str(iter)], 'test_err', 'test_crerr', 'train_err', 'train_crerr');
