...
load ('saved_variables/vh_f_5.mat')
load ('saved_variables/hp_f_5.mat')
load ('saved_variables/hp2_f_5.mat')
load ('saved_variables/hp3_f_5.mat')
load ('saved_variables/po_f_5.mat')
...
w5L=[hidtopL; toprecbiasesL];
w5R=[hidtopR; toprecbiasesR];
w6L=[hidtopL'; topgenbiasesL]; 
w6R=[hidtopR'; topgenbiasesR];
w7L=[hidpen3L'; hidgenbiases3L];
w7R=[hidpen3R'; hidgenbiases3R]; 
w8L=[hidpen2L'; hidgenbiases2L];
w8R=[hidpen2R'; hidgenbiases2R]; 
w9L=[hidpenL'; hidgenbiasesL];
w9R=[hidpenR'; hidgenbiasesR];
w10L=[vishidL'; visbiasesL];
w10R=[vishidR'; visbiasesR];
...
for batch = 1:numbatches
  ...
  dataoutL = 1./(1 + exp(-w9probs*w10L));
  dataoutR = 1./(1 + exp(-w9probs*w10R));
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  max_iter=3;
  VVL = [w1L(:)' w2L(:)' w3L(:)' w4L(:)' w5L(:)' w6L(:)' w7L(:)' w8L(:)' w9L(:)' w10L(:)']';  %%do one or both sets at once?
  VVR = [w1R(:)' w2R(:)' w3R(:)' w4R(:)' w5R(:)' w6R(:)' w7R(:)' w8R(:)' w9R(:)' w10R(:)']';  %%do one or both sets at once?
  ...
  [XL, XR, fX] = minimize2(VVL, VVR, 'CG_AE_f_5', max_iter, Dim, data);
  ...
  xxx = xxx+(l6+1)*l7;
  w7L = reshape(XL(xxx+1:xxx+(l7+1)*l8),l7+1,l8);
  w7R = reshape(XR(xxx+1:xxx+(l7+1)*l8),l7+1,l8);
  xxx = xxx+(l7+1)*l8;
  w8L = reshape(XL(xxx+1:xxx+(l8+1)*l9),l8+1,l9);
  w8R = reshape(XR(xxx+1:xxx+(l8+1)*l9),l8+1,l9);
  xxx = xxx+(l8+1)*l9;
  w9L = reshape(XL(xxx+1:xxx+(l9+1)*l10),l9+1,l10);
  w9R = reshape(XR(xxx+1:xxx+(l9+1)*l10),l9+1,l10);
  xxx = xxx+(l9+1)*l10;
  w10L = reshape(XL(xxx+1:xxx+(l10+1)*l11),l10+1,l11);
  w10R = reshape(XR(xxx+1:xxx+(l10+1)*l11),l10+1,l11);

%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%

  end

  %save mnist_weights5f w1L w1R w2L w2R w3L w3R w4L w4R w5L w5R w6L w6R w7L w7R w8L w8R w9L w9R w10L w10R
  %save mnist_error5f test_err train_err;
  save(['saved_variables/weights_f_5_' num2str(iter)], 'w1L', 'w1R', 'w2L', 'w2R', 'w3L', 'w3R', 'w4L', 'w4R', 'w5L', 'w5R', 'w6L', 'w6R', 'w7L', 'w7R', 'w8L', 'w8R', 'w9L', 'w9R', 'w10L', 'w10R');  
  save(['saved_variables/error_f_5_' num2str(iter)], 'test_err', 'train_err');
