...
%epsilonw      = 0.75;   % Learning rate for weights 
%epsilonvb     = 0.75;   % Learning rate for biases of visible units 
%epsilonhb     = 0.75;   % Learning rate for biases of hidden units 
%weightcost  = 0.0003;   
%initialmomentum  = 0.3;
%finalmomentum    = 0.9;
...
%%%%%%%%% START POSITIVE PHASE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  poshidprobsL = 1./(1 + exp(-data*vishidL - repmat(hidbiasesL,numcases,1))); %STEP 2
  poshidprobsR = 1./(1 + exp(-data*vishidR - repmat(hidbiasesR,numcases,1))); %STEP 2
...
%%%%%%%%% END OF POSITIVE PHASE  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  poshidstatesL = poshidprobsL > rand(numcases,numhid); %STEP 3
  poshidstatesR = poshidprobsR > rand(numcases,numhid); %STEP 5
  
  h0 = poshidprobsD;%(poshidstatesL + poshidstatesR) / 2;

%%%%%%%%% START NEGATIVE PHASE  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  negdataL = 1./(1 + exp(-poshidprobsL*vishidL' - repmat(visbiasesL,numcases,1))); %STEP 8
  negdataR = 1./(1 + exp(-poshidprobsR*vishidR' - repmat(visbiasesR,numcases,1))); %STEP 8
  %negdata = 1./(1 + exp(-poshidstates*vishid' - repmat(visbiases,numcases,1)));
  negdataD = (negdataL + negdataR)/2;
  
  neghidprobsL = 1./(1 + exp(-negdataD*vishidL - repmat(hidbiasesL,numcases,1)));   %STEP 14
  neghidprobsR = 1./(1 + exp(-negdataD*vishidR - repmat(hidbiasesR,numcases,1)));   %STEP 14
  ...
  h1 = neghidprobsD;%(neghidstatesL + neghidstatesR) / 2;
...
%%%%%%%%% UPDATE WEIGHTS AND BIASES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    ...
    deltaW = (data' * h0 - negdataD' * h1) / 2;%(posprods-negprods)/2;
    deltab = (sum(data) - sum(negdataD)) / 2; %(posvisact-negvisact)/2;
    deltac = (sum(h0) - sum(h1)) / 2;%(poshidact-neghidact)/2;
