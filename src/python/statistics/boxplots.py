...
def print_boxplot(title="foo", which="accuracy", all_data=np.empty([30, 5]), min_y=0.9, max_y=1):
    ...
    for tick, _ in zip(range(5), ax1.get_xticklabels()):
        k = tick % 2
...
def print_sns_boxplot(title="foo", which="accuracy", data=np.empty([30, 5]), min_y=0.9, max_y=1):
    ...
    df = df.melt(var_name='Number of Layers', value_name=which)
    fntDict = {'fontsize': 40, 'fontweight': 400, 'verticalalignment': 'baseline', 'horizontalalignment': 'center'}
    pd.concat([pd.concat([df], keys=['Number of Layers'], axis=1)], keys=[which])
    ...
    #which = 'Accuracy (%)' if which == "Accuracy" else which
    ax = sns.boxplot(x='Number of Layers', y=which, data=df)  # , width=0.3)
    sns.stripplot(x="Number of Layers", y=which, data=df, size=10, linewidth=2)
...
Configuration = input('(c)lassifier or (a)utoencoder:\n')
cf = input('(c)risp) or (f)uzzy:\n')
...
all_data = np.empty([30, 5])
counter = 0

for file in [f for f in os.listdir('reports') if f.startswith("result_" + Configuration + '_' + cf)]:
...
title = ("Crisp " if cf == "c" else "Fuzzy ") + ("Classifier" if Configuration == "classifier" else "Autoencoder")
if (which == "a"):
